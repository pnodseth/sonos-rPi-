"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var mongoose = require("mongoose");
var User = mongoose.model("User");
var Device = mongoose.model("Device");
var sonos_1 = require("../api/sonos");
exports.globalRFIDRegister = { test: "hei" };
function handleLoadPlaylist(message, user) {
    return __awaiter(this, void 0, void 0, function () {
        var data, room, rfid, userSecret, chip, device, response;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    data = JSON.parse(message);
                    room = data.room, rfid = data.rfid, userSecret = data.userSecret;
                    console.log("Got a request with room: " + room + " and rfid: " + rfid + " and user secret: " + userSecret);
                    chip = user.rfidChips.find(function (el) { return el.id === rfid; });
                    device = user.devices.find(function (el) { return el.deviceName === room; });
                    if (!(chip && device)) return [3 /*break*/, 2];
                    return [4 /*yield*/, sonos_1.startPlayback(device.sonosGroupId, chip.sonosPlaylistId, user)];
                case 1:
                    response = _a.sent();
                    console.log("response: ", response);
                    return [3 /*break*/, 3];
                case 2:
                    if (!chip) {
                        console.log("no chip found with id : ", rfid);
                    }
                    else if (!device) {
                        console.log("no device found with name: ", room);
                    }
                    _a.label = 3;
                case 3: return [2 /*return*/];
            }
        });
    });
}
exports.handleLoadPlaylist = handleLoadPlaylist;
function handlePlayback(message) {
    return __awaiter(this, void 0, void 0, function () {
        var data, room, command, userSecret;
        var _this = this;
        return __generator(this, function (_a) {
            data = JSON.parse(message);
            room = data.room, command = data.command, userSecret = data.userSecret;
            console.log("Got a request with room: " + room + " and command: " + command + " and user secret: " + userSecret);
            User.findOne({ userSecret: userSecret })
                .populate("devices")
                .exec(function (err, user) { return __awaiter(_this, void 0, void 0, function () {
                var device, response, data_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (!err) return [3 /*break*/, 1];
                            console.log(err);
                            return [3 /*break*/, 7];
                        case 1:
                            if (!user) return [3 /*break*/, 6];
                            device = user.devices.find(function (el) { return el.deviceName === room; });
                            if (!device) return [3 /*break*/, 4];
                            return [4 /*yield*/, sonos_1.togglePlayPause(device.sonosGroupId, command, user._id)];
                        case 2:
                            response = _a.sent();
                            console.log("response: ", response);
                            return [4 /*yield*/, response.json()];
                        case 3:
                            data_1 = _a.sent();
                            console.log("result: ", data_1);
                            return [3 /*break*/, 5];
                        case 4:
                            if (!device) {
                                console.log("no device found with name: ", room);
                            }
                            _a.label = 5;
                        case 5: return [3 /*break*/, 7];
                        case 6:
                            console.log("no user found with user secret: ", userSecret);
                            _a.label = 7;
                        case 7: return [2 /*return*/];
                    }
                });
            }); });
            return [2 /*return*/];
        });
    });
}
exports.handlePlayback = handlePlayback;
function handleSetDevice(message) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, userSecret, deviceName;
        return __generator(this, function (_b) {
            _a = JSON.parse(message), userSecret = _a.userSecret, deviceName = _a.deviceName;
            User.findOne({ userSecret: userSecret }, function (err, user) {
                if (err) {
                    console.log("error finding user with user secret: ", err);
                }
                if (!user) {
                    console.log("couldn't find user with user secret: ", userSecret);
                    //TODO: Send mqtt response back to blink LEDS or something
                }
                else {
                    // save new device
                    Device.findOne({ userSecret: userSecret, deviceName: deviceName }, function (err, device) {
                        if (err) {
                            console.log("error finding device: ", err);
                        }
                        else {
                            if (!device) {
                                device = new Device({
                                    userSecret: userSecret,
                                    deviceName: deviceName,
                                    user: user._id,
                                    sonosGroupId: ""
                                });
                                device.save(function (err) {
                                    if (err) {
                                        console.log("couldn't save device", err);
                                    }
                                });
                            }
                            if (!user.devices.includes(device._id)) {
                                user.devices.push(device._id);
                                user.save(function (err) {
                                    if (err) {
                                        console.log("TCL: err", err);
                                    }
                                    console.log("saved user with new device");
                                });
                            }
                        }
                    });
                }
            });
            return [2 /*return*/];
        });
    });
}
exports.handleSetDevice = handleSetDevice;
